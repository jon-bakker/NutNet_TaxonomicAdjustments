...
## Data release from November 15, 2022
...
source("scripts/Taxonomic.Adjustments.function.221115.R")

# 1.3 Load data ----
original.triplet <- read.csv("data/full-cover-2022-11-15.csv", header = T) # 296960 x 18
triplet <- original.triplet %>% # backup
  mutate(year = as.numeric(year))

original.site.covars <- read.csv("data/comb-by-plot-clim-soil-2022-11-15.csv",
                                 na.strings = c("NULL", "NA")) # 24216 x 87
site.covars.orig <- original.site.covars

adjustments.needed <- read.csv("data/taxonomic-adjustments-2022-11-15.csv")

# 2.0 PRE-PROCESSING OF COVER DATA ------------------------------------------------------------

## create summary table for each site showing composition BEFORE taxonomic adjustments
## (this only all sites)
sites.data <- unique(triplet$site_code) #143 sites
...
siteyear <- triplet %>%
  ...
  summarize(years = length(unique(year_trt)),
            ...
            .groups = "keep")
exptsites.all <- siteyear[siteyear$years >= 4 , ] # select sites with 4 or more years of data (n = 78)

# 2.2 Taxonomic QC ----
data1.all <- Taxonomic.Adjustments(datafile = triplet,
                                   adjustments.needed = adjustments.needed) # 252904 x 11
