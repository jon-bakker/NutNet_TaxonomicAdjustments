...
## Data release from January 31, 2025
...
source("scripts/Taxonomic.Adjustments.function.250131.R")

# 1.3 Load data ----
original.triplet <- read.csv("data/full-cover_2025-01-31.csv", header = T) # 351776 x 17
triplet <- original.triplet %>% # backup
  mutate(year = as.numeric(year))

original.site.covars <- read.csv("data/comb-by-plot-clim-soil_2025-01-31.csv",
                                 na.strings = c("NULL", "NA")) # 28780 x 87
site.covars.orig <- original.site.covars

adjustments.needed <- read.csv("data/taxonomic-adjustments-2025-01-31.csv")
...
siteyear <- triplet %>%
  ...
  summarize(years = length(unique(year)),
            ...
            .groups = "keep")
exptsites.all <- siteyear[siteyear$years >= 4 , ] # select sites with 4 or more years of data (n = 86)
...
data1.all <- Taxonomic.Adjustments(datafile = triplet,
                                   adjustments.needed = adjustments.needed) # 303790 x 13
