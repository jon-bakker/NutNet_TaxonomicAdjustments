...
source("scripts/Taxonomic.Adjustments.function.240531.R")

# 1.3 Load data ----
original.triplet <- read.csv("data/full-cover_2024-05-31.csv", header = T) # 348984 x 17
triplet <- original.triplet %>% # backup
  mutate(year = as.numeric(year))

original.site.covars <- read.csv("data/comb-by-plot-clim-soil_2024-05-31.csv",
                                 na.strings = c("NULL", "NA")) # 28549 x 87
site.covars.orig <- original.site.covars

adjustments.needed <- read.csv("data/taxonomic-adjustments-2024-05-31.csv")
...
data1.all <- Taxonomic.Adjustments(datafile = triplet,
                                   adjustments.needed = adjustments.needed) # 301229 x 13
